  ...
  useEffect(() => {
    return registerPage({
      ...
      apply: (name, args = {}) => {
        ...
        if (Number.isFinite(args.thetaDeg)) setThetaDeg(clamp(args.thetaDeg, 0.5, 89.5))
        if (Number.isFinite(args.m1)) setM1(clamp(args.m1, 0.05, 100))
        if (Number.isFinite(args.m2)) setM2(clamp(args.m2, 0.05, 100))
        if (Number.isFinite(args.muS)) setMuS1(clamp(args.muS, 0, 2))
        if (Number.isFinite(args.muK)) setMuK1(clamp(args.muK, 0, 2))
        if (Number.isFinite(args.Fapp)) setFapp(clamp(args.Fapp, -200, 200))
